...
public class GrilleStratego extends JPanel {
	...
	private Case[] tabBouton;			//permet de créer la grille de jeu
	private Principal p;
	...
	public void MouseClicked(java.awt.event.MouseEvent evt) {                                 
		...
		for(int i = 0; i<100; i++){
			if(bouton.getIcon() == tabBouton[i].getIcon() && temp!= -1){	//cas ou un pion a déjà été sélectionné et que le joueur souhaite le placer sur une case valide non occupée
				if(estDeplacementPossible(x, y, p.joueur.tableauPion[temp]) == true && deplacementPion(p.joueur.tableauPion[temp], x, y) == true){ //si le déplacement est possible
				...
				for(int j = 0; j<40; j++){		//cas ou la variable temporelle est nulle
					if(x == p.joueur.tableauPion[j].getX() && y == p.joueur.tableauPion[j].getY()){	// si on a clicker sur une case pion
						...
						System.out.println("Choisissez d'abord le pion a deplacer!");
							...
							if(deplacementPion(p.joueur.tableauPion[temp], x, y) == true){ // si le déplacement est possible
							...
							if(combat(temp, x, y)==1){	//si le joueur gagne
								p.joueur.tableauPion[temp].setX(x);	//son pion prend les coordonnées de l'adversaire
								p.joueur.tableauPion[temp].setY(y);
								tabBouton[i].setIcon(p.joueur.tableauPion[temp].getI());	//on mets l'image du pion du joueur à la place de celui de l'adversaire
